...
//    if(curr_node == nullptr)
//    {
//        leaf_node<K,V>* new_node = new leaf_node<K,V>(init_min, init_max);
//        if(phase == -1)
...
//    }
    if(curr_node -> is_leaf)
    {
        if(curr_node -> status > 0)
        {
            ...
            if(curr_node -> count.load(std::memory_order_seq_cst) >= curr_node->max)
            {
                ...
                Node<K,V>* new_node = new internal_node<K,V> (curr_node->min*1 , curr_node->max*1 );
    ...
    {
        curr_node -> mark();
        Node<K,V>* left_child = new internal_node<K,V> (curr_node->min, curr_node->max);
        Node<K,V>* right_child = new internal_node<K,V> (curr_node->min, curr_node->max);
    ...
    int64_t res;
    if(phase == -1)
        res = curr_node -> insert_leaf(key, value);
    else
        res = curr_node ->insert_leaf(key, value, tid, phase);
...
bool BPTree<K,V>::remove(K key, int tid, int phase) {
    ...
    if(curr_node -> is_leaf)
    {
        if(curr_node -> status == 0) {
            int64_t result;
            if(phase == -1)
                result = curr_node -> delete_leaf(key);
            else
                result = curr_node -> delete_leaf(key, tid, phase);
            if (result == 1) {
                size--;
                return true;
            } else if (result == 0) {
    ...
    while(!curr_node -> is_leaf)
    {
        ...
        if(curr_child -> is_leaf && curr_child -> status > 0)
        {
            ...
            if(curr_child -> count.load(std::memory_order_seq_cst) >= curr_child -> max) //Split the Current Child
            {
                curr_child -> split_leaf();
                Node<K,V>* new_node = new internal_node<K,V>(curr_node -> min, curr_node -> max);
            ...
            else if(curr_child -> count < curr_child -> min)  // Merge the current child
            {
                int64_t ls_idx = curr_idx - 1; // Left_Sibling Index
                int64_t rs_idx = curr_idx + 1; // Right Sibling Index
                if(ls_idx >= 0)        // Left Sibling Exist
    ...
    int64_t res;
    if(phase == -1)
        res = curr_node -> delete_leaf(key);
    else
        res = curr_node -> delete_leaf(key, tid, phase);
    if(res == 1){
        size--;
        return true;
    }
...
Node<K, V>* BPTree<K,V> :: do_helping(Node<K, V>* prev_node, int64_t prev_idx, Node<K, V>* curr_node) {
    ...
    if(curr_child -> is_leaf) {
        if (curr_child -> count >= curr_child -> max) //Split the Current Child
        {
            curr_child -> split_leaf();
            Node<K,V>* new_node = new internal_node<K,V>(curr_node -> min, curr_node -> max);
            Node<K,V>* left_child = curr_child -> new_next.load(std::memory_order_seq_cst);
