...
#define RQ_DEBUGGING_H
    ...
    #define MAX_NUM_RQ_IN_EXECUTION (1<<20)
        ...
        void printLogarithmicHistogram(T * valuesOverTime, int numValues) {
            ...
            for (int i=0;i<=numBits;++i) {
                if (histogram[i] > 0) {
                    cout<<"    (2^"<<twoDigits(i)<<", 2^"<<twoDigits(i+1)<<"]: "<<histogram[i]<<std::endl;
                }
            }
            cout<<"    average = "<<(sum / (double) cntNonZero)<<std::endl;
        }
        ...
        inline void DEBUG_RECORD_UPDATE_CHECKSUM(const int tid, const long long timestamp, Node * const * const insertedNodes, Node * const * const deletedNodes, DataStructure * const ds) {
            ...
            if (timestamp >= MAX_NUM_RQ_IN_EXECUTION) {
                return;
    //            cout << "timestamp is: " << timestamp << std::endl;
    //            error("timestamp > MAX_NUM_RQ_IN_EXECUTION");
        ...
        inline void DEBUG_RECORD_RQ_CHECKSUM(const int tid, const long long timestamp, K const * const rqResult, const int len) {
            ...
            if (timestamp < 5) {
                cout<<"timestamp="<<timestamp<<" len="<<len<<std::endl; ///////////////////////////////////////////
                cout<<std::endl;
            }
#endif
            for (int i=0;i<len;++i) {
        ...
        void DEBUG_VALIDATE_RQ(const int numProcesses) {
            ...
            for (int timestamp=0;timestamp<MAX_NUM_RQ_IN_EXECUTION;++timestamp) {
                for (int tid=0;tid<numProcesses;++tid) {
                    //if (threadUpdateChecksum[tid][timestamp]) cout<<"threadUpdateChecksum[tid="<<tid<<", timestamp="<<timestamp<<"]="<<threadUpdateChecksum[tid][timestamp]<<std::endl;
...
#ifdef DEBUG_RQ_PROVIDER_METRICS
                    if (timestamp < 5) {
                        if (threadUpdateChecksum[tid][timestamp]) cout<<"tid="<<tid<<" timestamp="<<timestamp<<" threadUpdateChecksum="<<threadUpdateChecksum[tid][timestamp]<<std::endl; //////////////////////////////////////////////
                        if (threadRQChecksum[tid][timestamp]) cout<<"tid="<<tid<<" timestamp="<<timestamp<<" threadRQChecksum="<<threadRQChecksum[tid][timestamp]<<std::endl; //////////////////////////////////////////////
                    }
#endif
                }
#ifdef DEBUG_RQ_PROVIDER_METRICS
                if (timestamp < 5) {
                    if (updateChecksum[timestamp]) cout<<"updateChecksum[timestamp="<<timestamp<<"]="<<updateChecksum[timestamp]<<std::endl; ////////////////////////////////////////////////
                    if (rqChecksum[timestamp]) cout<<"rqChecksum[timestamp="<<timestamp<<"]="<<rqChecksum[timestamp]<<std::endl; ////////////////////////////////////////////////
                }
#endif
                ...
                for (int timestamp=0;timestamp<MAX_NUM_RQ_IN_EXECUTION;++timestamp) {
                    if (threadRQChecksum[tid][timestamp] != NO_RQ_CHECKSUM) {
                        if (threadRQChecksum[tid][timestamp] != prefixSum) {
                            ++numberFailed;
                            if (numberFailed < 100) {
                                cout<<"RQ VALIDATION ERROR: threadRQChecksum[tid="<< tid <<"][timestamp="<<timestamp<<"]="<<threadRQChecksum[tid][timestamp]<<" is not equal to prefixSum=updateChecksum[0, 1, ..., timestamp-1]="<<prefixSum<<std::endl;
                            } else if (numberFailed == 100) {
                                cout<<"RQ VALIDATION: too many errors to list..."<<std::endl;
                            }
                            good = false;
                            //exit(-1);
                        } else {
            ...
            for (int timestamp=0;timestamp<MAX_NUM_RQ_IN_EXECUTION;++timestamp) {
                if (rqChecksum[timestamp] != NO_RQ_CHECKSUM) {
                    if (rqChecksum[timestamp] != prefixSum) {
                        ++numberFailed;
                        if (numberFailed < 100) {
                            cout<<"RQ VALIDATION ERROR: rqChecksum[timestamp="<<timestamp<<"]="<<rqChecksum[timestamp]<<" is not equal to prefixSum=updateChecksum[0, 1, ..., timestamp-1]="<<prefixSum<<std::endl;
                        } else if (numberFailed == 100) {
                            cout<<"RQ VALIDATION: too many errors to list..."<<std::endl;
                        }
                        good = false;
    ...
    #endif
            if (numberFailed > 0) {
                cout<<"RQ VALIDATION TOTAL FAILURES: "<<numberFailed<<std::endl;
                cout<<"    (note: validation only works for RQs over the entire data structure)"<<std::endl;
            }
            cout<<"RQ VALIDATION TOTAL SUCCESSES: "<<numberSucc<<std::endl;
            cout<<"    (note: this captures only non-empty RQs, and is at most "<<MAX_NUM_RQ_IN_EXECUTION<<")"<<std::endl;
            if (good) cout<<"RQ Validation OK"<<std::endl;
            cout<<std::endl;

    #ifdef RQ_VISITED_IN_BAGS_HISTOGRAM
            cout<<"histogram: how many RQs visited x nodes in limbo bags?"<<std::endl;
