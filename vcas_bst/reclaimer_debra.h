...
//        SOFTWARE_BARRIER;
    ...
    void debugPrintStatus(const int tid) {
        if (tid == 0) {
            cout<<"global epoch counter="<<epoch<<std::endl;
        }
    }
...
//        }
    ...
    reclaimer_debra(const int numProcesses, Pool *_pool, debugInfo * const _debug, RecoveryMgr<void *> * const _recoveryMgr = NULL)
        ...
        VERBOSE cout<<"constructor reclaimer_debra helping="<<this->shouldHelp()<<std::endl;// scanThreshold="<<scanThreshold<<std::endl;
        epoch = 0;
    ...
    ~reclaimer_debra() {
        VERBOSE DEBUG cout<<"destructor reclaimer_debra"<<std::endl;
        for (int tid=0;tid<this->NUM_PROCESSES;++tid) {
            for (int i=0;i<NUMBER_OF_EPOCH_BAGS;++i) {
