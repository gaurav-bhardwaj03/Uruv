...
//        VERBOSE DEBUG2 COUTATOMIC("record_manager_single_type::enterQuiescentState(tid="<<tid<<")"<<std::endl);
    ...
    inline void leaveQuiescentState(const int tid, void * const * const reclaimers, const int numReclaimers, const bool readOnly = false) {
//        assert(isQuiescent(tid));
        reclaim->leaveQuiescentState(tid, reclaimers, numReclaimers, readOnly);
    }
