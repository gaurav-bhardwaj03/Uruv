...
#ifdef BLOCKBAG_ITERATOR_COUNT_STEPS
            if (ix < 0) cout<<"bag="<<bag<<" head="<<head<<" curr="<<curr<<" ix="<<ix<<" steps="<<steps<<std::endl;
#endif
        ...
        inline blockbag_iterator<T>& operator++(int) {
            ...
            if (ix < 0) {
                ...
                if (blocksTraversed > sizeInBlocks + 1) {
                    cout<<"ERROR: too many blocks traversed! traversed "<<blocksTraversed<<" when we expected at most 1+"<<sizeInBlocks<<std::endl;
                    exit(-1);
                }
...
//            VERBOSE DEBUG cout<<"constructor blockbag"<<std::endl;
            owner = tid;
//            cout<<"bag owner="<<owner<<std::endl;
            reclaimCount = 0;
            debugFreed = 0;
            sizeInBlocks = 1;
...
//            VERBOSE DEBUG cout<<" freed "<<debugFreed<<std::endl;
        }
...
//                MEMORY_STATS2 alloc->debug->addFromPool(tid, 1);
        ...
        block<T>* removeFullBlock() {
            ...
            DEBUG2 if (sizeInBlocks != computeSizeInBlocks()) { cout<<"sizeInBlocks="<<sizeInBlocks<<" compute="<<computeSizeInBlocks()<<std::endl; }
            DEBUG2 assert(sizeInBlocks == computeSizeInBlocks());
            DEBUG2 validate();
            return 0;
        }
