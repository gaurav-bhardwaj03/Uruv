...
public:
    ...
    inline bool protect(const int tid, T * const obj, CallbackType notRetiredCallback, CallbackArg callbackArg, bool memoryBarrier = true) {
        TRACE cout<<"reclaimer_hazardptr::protect(tid="<<tid<<", "<<debugPointerOutput(obj)<<")"<<std::endl;
...
//            SOFTWARE_BARRIER;
            TRACE cout<<"notRetiredCallback returns true"<<std::endl;
...
//            SOFTWARE_BARRIER;
            ...
            TRACE cout<<"notRetiredCallback returns false"<<std::endl;
...
//            SOFTWARE_BARRIER;
    ...
    inline void unprotect(const int tid, T * const obj) {
        TRACE cout<<"reclaimer_hazardptr::unprotect(tid="<<tid<<", "<<debugPointerOutput(obj)<<")"<<std::endl;
//        SOFTWARE_BARRIER;
        DEBUG2 assert(isProtected(tid, obj));
        int size; DEBUG2 size = announce[tid]->size();
...
//        SOFTWARE_BARRIER;
    ...
    inline bool qProtect(const int tid, T * const obj, CallbackType notRetiredCallback, CallbackArg callbackArg, bool memoryBarrier = true) {
        TRACE cout<<"reclaimer_debraplus::qProtect(tid="<<tid<</*", "<<*obj<<*/")"<<std::endl;
        return false;
    }
    inline void qUnprotectAll(const int tid) {
        TRACE cout<<"reclaimer_debraplus::qUnprotectAll(tid="<<tid<<")"<<std::endl;
    }
    
    // for epoch based reclamation
    inline void enterQuiescentState(const int tid) {
        TRACE cout<<"reclaimer_hazardptr::enterQuiescentState(tid="<<tid<<")"<<std::endl;
//        SOFTWARE_BARRIER;
        announce[tid]->clear();
//        __sync_synchronize();
...
//        SOFTWARE_BARRIER;
    ...
    inline static bool leaveQuiescentState(const int tid, void * const * const reclaimers, const int numReclaimers) {
        TRACE cout<<"reclaimer_hazardptr::leaveQuiescentState(tid="<<tid<<")"<<std::endl;
//        SOFTWARE_BARRIER;
        return false;
    }
    ...
    inline void retire(const int tid, T* p) {
        TRACE cout<<"reclaimer_hazardptr::retire(tid="<<tid<<", "<<debugPointerOutput(p)<<")"<<std::endl;
...
//                }
//            }
//            TRACE cout<<"    afterwards, we have "<<retired[tid]->size()<<" things waiting to be retired..."<<std::endl;
...
//        assert(tid < this->NUM_PROCESSES);
    ...
    reclaimer_hazardptr(const int numProcesses, Pool *_pool, debugInfo * const _debug, RecoveryMgr<void *> * const _recoveryMgr = NULL)
        ...
        VERBOSE DEBUG cout<<"constructor reclaimer_hazardptr"<<std::endl;
    ...
    ~reclaimer_hazardptr() {
        VERBOSE DEBUG cout<<"destructor reclaimer_hazardptr"<<std::endl;
