...
public:
    ...
    inline void leaveQuiescentState(const int tid, const bool callForEach, const bool readOnly = false) {}
};
...
public:
    ...
    inline void leaveQuiescentStateForEach(const int tid, const bool readOnly = false) {
        mgr->leaveQuiescentState(tid, NULL, 0, readOnly);
    ...
    inline void leaveQuiescentState(const int tid, const bool callForEach, const bool readOnly = false) {
        if (callForEach) {
            ...
            get((First *) NULL)->leaveQuiescentState(tid, reclaimers, 1+sizeof...(Rest), readOnly);
...
//            cout<<"setting quiescent state for just one record type: "<<typeid(RecordTypesFirst).name()<<"\n";
    ...
    inline void leaveQuiescentState(const int tid, const bool readOnly = false) {
...
//        VERBOSE DEBUG2 COUTATOMIC("record_manager_single_type::leaveQuiescentState(tid="<<tid<<")"<<std::endl);
        ...
        rmset->leaveQuiescentState(tid, Reclaim::quiescenceIsPerRecordType(), readOnly);
    }
